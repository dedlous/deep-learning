...
def get_init_cell(batch_size, rnn_size):
    ...
    cell = tf.contrib.rnn.MultiRNNCell([make_cell(rnn_size) for _ in range(3)])  
...
def get_batches(int_text, batch_size, seq_length):
    ...
    batch_count = len(int_text)//chars_per_batch
    
    batches = np.zeros((batch_count, 2, batch_size, seq_length))
    for b_num in range(batch_count):
        for i in range(batch_size):
            idx = batch_count * i + b_num
            batches[b_num][0][i] = int_text[seq_length * idx : seq_length * (idx + 1)]
            batches[b_num][1][i] = int_text[seq_length * idx + 1 : seq_length * (idx + 1) + 1]
